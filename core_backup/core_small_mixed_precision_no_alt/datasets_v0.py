...
class DICDataset(FlowDataset):
    ...
    def __init__(self, aug_params=None, split='training', root='G:/20251017_RAFTcorr_training_dataset_20000'):
        # For 128×128 images, adjust augmentation to avoid crop size issues
        if aug_params is not None and 'crop_size' in aug_params:
            # If crop_size is >= 128, use smaller size or disable augmentation
            crop_h, crop_w = aug_params['crop_size']
            if crop_h >= 128 or crop_w >= 128:
                print(f"Note: Adjusting crop_size from {aug_params['crop_size']} to [112, 112] for 128×128 images")
                aug_params['crop_size'] = [112, 112]
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    # DIC dataset
    if args.stage == 'dic':
        # For 128×128 images, use minimal augmentation (no scaling/cropping)
        if args.image_size[0] == 128 and args.image_size[1] == 128:
            # No spatial augmentation, only color and flip
            aug_params = {'crop_size': args.image_size, 'min_scale': 0.0, 'max_scale': 0.0, 'do_flip': True}
        else:
